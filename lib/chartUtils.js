export function buildChartData(history, tradable, crowdlending, rates) {
  ...
  // When no stock history exists (crowdlending-only view), generate timestamps from payment dates
  if (allTs.size === 0 && crowdlending.length > 0) {
...
export function buildComparisonData(history, tradable, crowdlending, rates, benchmarkSymbol) {
